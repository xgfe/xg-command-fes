...
exports = module.exports = function(argv) {
    return generateOption(argv).then(function (option) {
        ...
        console.log('[FES] cwd:         ', ctx.cwd);
        console.log('[FES] config:      ', ctx.type, ctx.raw);
        console.log('[FES] entry:       ', ctx.package.module_entry);
        console.log('[FES] dependencies:', Object.keys(ctx.package.module_dependencies).join(','));
        console.log('[FES] Checkout Entry:', ctx.module_entry.name + '@' + ctx.module_entry.version);
        return ctx.module_entry.initialize().then(function (sha) {
            ...
            return promiseSerial(ctx.module_dependencies.map(function (mod) {
                return function () {
                    console.log(`[FES] Install Module: ${mod.name}@${mod.version}`);
                    console.log(`              target: ${mod.target}`);
                    console.log(`            resolved: ${mod.resolved}`);
                    return mod.install().then((sha => {
                        console.log(`               commit: ${sha}`);
                        const pkgInfo = mod.info();
                        if (mod.name !== pkgInfo.name) {
                            throw new Error(
                                `The name of FES(${mod.name}) and package.json(${pkgInfo.name}) of the ${mod.name} must be the same`
                            );
                        }
                    }));
                };
            ...
            console.log('[FES] dynamic-dependence');
            console.log('====================================');
